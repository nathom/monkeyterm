...
static REMOTE_CACHE: OnceLock<Mutex<HashMap<String, String>>> = OnceLock::new();
...
fn cache_dir() -> Option<std::path::PathBuf> {
    dirs::cache_dir().map(|d| d.join("monkeyterm").join("languages"))
}

/// Fetch a remote language word list. Checks disk cache first, then downloads.
fn fetch_remote_language(name: &str) -> Option<String> {
    ...
    if let Some(dir) = cache_dir() {
        ...
        if let Ok(data) = std::fs::read_to_string(&path) {
            if !data.is_empty() {
                let mut cache = remote_cache().lock().ok()?;
    ...
    if let Some(dir) = cache_dir() {
        ...
        let _ = std::fs::write(dir.join(format!("{}.txt", name)), &word_list);
    }

    // Save to memory cache
    if let Ok(mut cache) = remote_cache().lock() {
        cache.insert(name.to_string(), word_list.clone());
    }
...
pub fn is_remote_language(name: &str) -> bool {
    REMOTE_LANGUAGE_NAMES.contains(&name)
}

fn get_word_list_owned(language: &str) -> Vec<String> {
    // Try embedded first
    ...
    // Try remote
    if let Some(data) = fetch_remote_language(language) {
        return data
            .lines()
            .filter(|l| !l.is_empty())
            .map(|s| s.to_string())
...
pub fn generate_words(
    ...
    let word_list = get_word_list_owned(language);
    if word_list.is_empty() {
...
mod tests {
    ...
    fn test_no_consecutive_repeats() {
        ...
        for i in 1..words.len() {
            assert_ne!(words[i].to_lowercase(), words[i - 1].to_lowercase());
        }
    }
    ...
    fn test_embedded_languages_load() {
        for lang in ["english", "english_1k", "french", "german", "spanish", "turkish", "japanese_romaji", "korean"] {
            let words = get_word_list_owned(lang);
    ...
    fn test_code_snippets_load() {
        for lang in code_language_names() {
            ...
            assert!(lines[0] != "// no code snippets available");
        }
    }
